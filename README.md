...
正式開發分支：`feat/nextjs-portfolio`。目前完成階段 1–3，等待確認後再進行完整內容搬遷。
...
- Notebook 索引與 SSG 文章頁，先放 4 篇原文、9 張原始圖片。
...
UI 使用原生語意元素與自訂樣式；沒有套入 shadcn 的預設視覺主題。複雜互動日後有需要再加入 Base UI。
...
其餘 Notebook 內容、Work 六個分類的正式內容、舊 `/docs/` URL 相容、Home / Contact 正式文案與正式 SEO。26 篇 Blog 不在目前遷移範圍。預览版本暫設 noindex。
